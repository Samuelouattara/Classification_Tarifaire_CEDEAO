...
export class TariffAIClassifier {
  ...
  private analyzeContext(description: string): any {
    return {
      state: "unknown",
      usage: "unknown",
      processing: "unknown",
  ...
  private detectNegations(description: string): any[] {
    return [];
  }
  ...
  private applyContextualRules(analysis: any, scores: Record<string, any>): void {
    // Implémentation des règles contextuelles
  }

  private calculateFinalScores(scores: Record<string, any>): void {
    const maxScore = Math.max(...Object.values(scores).map((s: any) => s.score));
  ...
  private calculateConfidence(scoreData: any): number {
